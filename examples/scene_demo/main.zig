...
pub fn main() !void {
    ...
    var cam_transform = TransformComponent.withPosition(Vec3.init(0, 3, 6));
...
fn update(scene: *Scene, input: *Input, delta_time: f32) !void {
    ...
    if (scene.getComponent(TransformComponent, camera_entity)) |cam_transform| {
        ...
        if (wasd.x != 0 or wasd.y != 0) {
            const move = Vec3.init(wasd.x * speed * delta_time, 0, wasd.y * speed * delta_time);
            cam_transform.translate(move);
        }
